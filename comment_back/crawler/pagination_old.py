...
class OptionalCountPagination(PageNumberPagination):
    ...
    def paginate_queryset(self, queryset, request, view=None):
        ...
        if include_count:
            ...
            page_size_int = int(page_size) if isinstance(page_size, str) else page_size
            ...
            page_size_int = int(page_size) if isinstance(page_size, str) else page_size
            self.page = self._get_page_without_count(queryset, page_number, page_size_int)
    ...
    def get_paginated_response(self, data):
        ...
        # include_count가 True일 때만 count 추가
        if include_count and hasattr(self.page, 'paginator') and hasattr(self.page.paginator, 'count'):
...
class OptionalCountLimitOffsetPagination(PageNumberPagination):
    """
    LimitOffset 방식의 페이지네이션으로, include_count 파라미터가 있을 때만 전체 개수를 계산합니다.
    """
    ...
    def get_paginated_response(self, data):
        ...
        if include_count:
            response_data["count"] = self.page.paginator.count
            response_data.move_to_end("results")  # results를 마지막으로 이동

        return Response(response_data)

    def paginate_queryset(self, queryset, request, view=None):
        self.request = request  # request 객체 저장
